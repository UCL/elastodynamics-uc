...
def SolveProblem(msh,order=1,add_bc=False,export_VTK=False,type_str="oscillatory",pgamma=None): 
    #msh = mesh.create_rectangle(comm=MPI.COMM_WORLD,
    #                            points=((0.0, 0.0), (1.0, 1.0)), n=(N_mesh, N_mesh),
    #                            cell_type=mesh.CellType.triangle,)

    ue,f,pgamma_,palpha = get_reference_sol(msh,type_str)
    if not pgamma:
        pgamma = pgamma_
...
#for add_bc,problem_type,pgamma in zip([True,False],["well-posed","ill-posed"],[ ScalarType(1e-5),ScalarType(1e-4)]):
for add_bc,problem_type,pgamma in zip([True,False],["well-posed","ill-posed"],[ ScalarType(1e-5),ScalarType(1e-1)]):
    ...
    #for N_mesh in Ns: 
    for msh in ls_mesh:
        l2_error, ndof = SolveProblem(msh=msh,order=order,add_bc=add_bc,pgamma=pgamma)
